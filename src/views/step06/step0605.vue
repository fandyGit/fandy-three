...
onMounted(() => {
  ...
  function createMesh(geom:any) {
    ...
    let meshMaterial = new THREE.MeshPhongMaterial({color:0x333333,shininess:100,metal:true})
  ...
  let shape = createMesh(new THREE.ShapeGeometry(drawerShape()))
  scene.add(shape)
  ...
  spotLight.position.set(10,100,70)
  ...
  let controls = new function () {
    ...
    this.draw = function () {
      ...
      shape = createMesh(new THREE.ExtrudeGeometry(drawerShape(),options))
      scene.add(shape)
    }
  }

  let gui = new dat.GUI()
  gui.add(controls,'amount',1,20).onChange(controls.draw)
  gui.add(controls,'bevelThickness',1,20).onChange(controls.draw)
  gui.add(controls,'bevelSegments',1,20).step(1).onChange(controls.draw)
  gui.add(controls,'curveSegments',1,20).step(1).onChange(controls.draw)
  gui.add(controls,'bevelSize',1,20).onChange(controls.draw)
  gui.add(controls,'bevelEnabled').onChange(controls.draw)
  gui.add(controls,'steps',1,20).step(1).onChange(controls.draw)
